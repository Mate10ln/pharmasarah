      ...
      <Modal isOpen={isModalOpen} onClose={handleCloseModal} title={editingProduct ? "Edit Product" : "Add New Product"}>
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <input name="name" type="text" placeholder="Product Name" value={productData.name} onChange={handleInputChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-brand-blue focus:border-brand-blue"/>
            <input name="category" type="text" placeholder="Category (Optional)" value={productData.category} onChange={handleInputChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-brand-blue focus:border-brand-blue"/>
            {editingProduct ? (
                <input name="sku" type="text" placeholder="SKU (Unique)" value={productData.sku} readOnly className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-100 text-gray-500 cursor-not-allowed"/>
            ) : (
                <div className="w-full px-3 py-2 border border-gray-200 rounded-md bg-gray-50 text-gray-500">SKU will be auto-generated</div>
            )}
            <div className="relative">
                <input name="barcode" type="text" placeholder="Barcode (Optional)" value={productData.barcode || ''} onChange={handleInputChange} className="w-full pl-3 pr-10 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-brand-blue focus:border-brand-blue"/>
                <button type="button" onClick={() => setScannerOpen(true)} className="absolute inset-y-0 right-0 pr-3 flex items-center text-gray-500 hover:text-brand-blue" title="Scan Barcode"><CameraIcon className="h-5 w-5"/></button>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <input name="costPrice" type="number" placeholder="Cost Price" value={productData.costPrice || ''} onChange={handleInputChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-brand-blue focus:border-brand-blue"/>
            <input name="price" type="number" placeholder="Sell Price" value={productData.price || ''} onChange={handleInputChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-brand-blue focus:border-brand-blue"/>
            <input name="quantity" type="number" placeholder="Quantity" value={productData.quantity || ''} onChange={handleInputChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-brand-blue focus:border-brand-blue"/>
            <input name="lowStockThreshold" type="number" placeholder="Low Stock Threshold" value={productData.lowStockThreshold || ''} onChange={handleInputChange} className="w-full px-3 py-2 border border-gray-300 rounded-md focus:outline-none focus:ring-brand-blue focus:border-brand-blue"/>
          </div>
          <button
            onClick={handleSaveProduct}
            className="w-full py-2 px-4 bg-brand-blue text-white rounded-md hover:bg-brand-blue-dark transition-colors"
          >
            Save Product
          </button>
        </div>
